...
for i in years:
    if i == '2016':
        for j in range(0, len(month)):
            data = post_elexon('https://api.bmreports.com/BMRS/MELIMBALNGC/v1?APIKey=b297kkm9vfw946g&ZoneIdentifier=N&FromDate={}-{}-01&ToDate={}-{}-{}&ServiceType=xml'.format(i, month[j], i, month[j], days_16[j]))
            # atach useful data
        ...
        for j in range(0, len(month)):
            data = post_elexon('https://api.bmreports.com/BMRS/MELIMBALNGC/v1?APIKey=b297kkm9vfw946g&ZoneIdentifier=N&FromDate={}-{}-01&ToDate={}-{}-{}&ServiceType=xml'.format(i, month[j], i, month[j], days[j]))
            # atach useful data
